...
File& File::getname(File& x)const {return filename;} //Получаем имя файла
File& File::gettype(File& x)const {return filetype;} //Получаем тип файла
int File::getsize(File& x)const {return filesize;} //Получаем размер файла 
File& File::getcreator(File& x)const {return filecreator;} //Получем имя создателя
int File::gettimec(File& x)const {return timeofcr;} //Получаем время создания
int File::gettimem(File& x)const {return timeofmod;} //Получаем время модификации
